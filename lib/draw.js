var Game = require('./game');

var Draw = function(){
  var canvas = document.getElementById('screen');
  this.context = canvas.getContext('2d');
  var gameSize = { x: canvas.width, y: canvas.height};

  var self = this;
  self.game = new Game(gameSize);

  var tick = function(){
    // self.update(self.game);
    self.game.update()
    // self.draw(screen, gameSize);
    requestAnimationFrame(tick);
    self.context.clearRect(0, 0, 600, 300)
    self.drawTank(self.context, self.game)
    self.drawAlien(self.context, self.game)
  };
  tick();

  // window.onkeydown = function(e) {
  //   if(e.keyCode === 37){
  //     self.game.update();
  //
  //   }
  //   keyState[e.keyCode] = true;
  // };
  //
  // window.onkeyup = function(e) {
  //   keyState[e.keyCode] = false;
  // };


};

Draw.prototype.drawTank = function (context, game) {
  context.fillRect(game.bodies[0].x, game.bodies[0].y, game.bodies[0].size.x, game.bodies[0].size.y);
  // return this;
};

Draw.prototype.drawAlien = function (context, game) {
  debugger
  game.bodies[1].forEach(function(alien) {
    context.fillRect(alien.x, alien.y, alien.size.x, alien.size.y)
  })
  // context.fillRect(game.bodies[1].x, game.bodies[1].y, game.bodies[1].size.x, game.bodies[1].size.y);
  // return this;
};

Draw.prototype.update = function (game) {
  // for (var i=0; i<game.bodies.length; i++) {

    game.update();
  // }
};


Draw.prototype.draw = function (screen, gameSize) {
  // Draw.prototype.drawTank(this.context, this.game);
  // Draw.prototype.drawAlien(this.context, this.game);

};

module.exports = Draw;
